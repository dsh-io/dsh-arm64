...
# Keep a CI-restored cache alive across the stage wipe.
if [ -f "${STAGE}/rootfs-cache.tar.gz" ]; then
  mv "${STAGE}/rootfs-cache.tar.gz" "${STAGE}.cache.tar.gz"
fi
rm -rf "${STAGE}"
mkdir -p "${STAGE}/rootfs" "${STAGE}/work"
[ -f "${STAGE}.cache.tar.gz" ] && mv "${STAGE}.cache.tar.gz" "${STAGE}/rootfs-cache.tar.gz"
cd "${STAGE}"

# CI cache fast path: the trimmed rootfs (node + overlay + mirrors + workspace)
# is cached; when present, skip debootstrap + downloads entirely.
CACHE_TAR="${STAGE}/rootfs-cache.tar.gz"
if [ -f "${CACHE_TAR}" ]; then
  echo "==> cache hit: restoring rootfs from ${CACHE_TAR}"
  tar -xzf "${CACHE_TAR}" -C "${STAGE}"
  [ -x rootfs/usr/local/bin/node ] || { echo "!! cached rootfs missing node" >&2; exit 1; }
  [ -f rootfs/root/.dsh-arm64/node_modules/@deepseek-ai/dsh/lib/bin.js ] \
    || { echo "!! cached rootfs missing dsh" >&2; exit 1; }
else
    echo "==> cache miss: full build"
  {
...
# ---- 7. cache the trimmed rootfs for the next CI run ----
tar -czf "${CACHE_TAR}" -C "${STAGE}" rootfs
ls -lh "${CACHE_TAR}"

# ---- 8. package rootfs.tar.xz + sha256 ----
